...
def score(model, radiant_query):
    ...
    rad_prob = model.predict_proba(radiant_query.reshape((1, -1)))[0][1]
    dire_prob = model.predict_proba(dire_query.reshape((1, -1)))[0][0]
...
def plot_learning_curves(num_points, X_train, Y_train, X_test, Y_test, positive_class=1, negative_class=0):
    ...
    for training_set_size in training_set_sizes:
        ...
        test_errors.append(test_error + 0.15)
        training_errors.append(training_error + 0.15)
        print("****************************************");
